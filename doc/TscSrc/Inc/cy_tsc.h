 ...
 * Author: sk
 *
 * Created on 2013Âπ?Êú?2Êó? ‰∏ãÂçà4:33
 */
...
typedef struct Unit_t
{
	...
	unsigned char		sntime;
	unsigned char	 	ewtime;
...
//	unsigned char  		PhaseOffset;
	unsigned short		PhaseOffset; //added on 20150413
...
//Detector Data Table
typedef struct DetectorDataList_t
{
...
{
	unsigned short			DataId;
	unsigned short			FactFlowNum;
	DetectorDataList_t		DetectorDataList[FLOWNUM];
}DetectorData_t;
...
typedef struct lamperrList_t
{
	unsigned char		chanid;
	unsigned char		etype;
}lamperrList_t;

typedef struct lamperrtype_t
{
	lamperrList_t	lamperrList[MAX_CHANNEL_EX];
}lamperrtype_t;
