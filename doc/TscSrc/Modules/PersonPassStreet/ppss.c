...
int pps_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,ppspinfo_t *pinfo)
{
...
int pps_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int pps_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef PERSON_DEBUG
...
int pps_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef PERSON_DEBUG
...
int pps_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],ppspinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef PERSON_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef PERSON_DEBUG
				...
				if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef PERSON_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef PERSON_DEBUG
			...
			if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef PERSON_DEBUG
		...
		{
			if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef PERSON_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef PERSON_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int pps_jump_stage_control(markdata_c *mdt,unsigned char staid,ppspinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef PERSON_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef PERSON_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != pps_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef PERSON_DEBUG
...
void pps_red_flash(void *arg)
{
	...
	if (0 == (rdt->pps->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->pps->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->pps->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->pps->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->pps->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->pps->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->pps->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->pps->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->pps->fd->specfunc) & 0x10) && (*(rdt->pps->fd->specfunc) & 0x20))
            {
                tchan = rdt->pps->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->pps->fd->specfunc) & 0x20) && (!(*(rdt->pps->fd->specfunc) & 0x10)))
            {
                tchan = rdt->pps->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->pps->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->pps->fd->markbit) & 0x1000)) && (!(*(rdt->pps->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->pps->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != pps_set_lamp_color(*(rdt->pps->fd->bbserial),cchan,0x03))
		{
		#ifdef PERSON_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if(SUCCESS!=update_channel_status(rdt->pps->fd->sockfd,rdt->pps->cs,cchan,0x03,rdt->pps->fd->markbit))
		{
		#ifdef PERSON_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != pps_set_lamp_color(*(rdt->pps->fd->bbserial),cchan,0x00))
		{
		#ifdef PERSON_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->pps->fd->sockfd,rdt->pps->cs,cchan,0x00,rdt->pps->fd->markbit))
		{
		#ifdef PERSON_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->pps->fd->markbit) & 0x1000)) && (!(*(rdt->pps->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->pps->fd->sockfd->csockfd);
    }
...
int pps_mobile_jp_control(markdata_c *mdt,unsigned char staid,ppspinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef PERSON_DEBUG
...
int pps_mobile_direct_control(markdata_c *mdt,ppspinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef PERSON_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != pps_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef PERSON_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
void pps_channel_yellow_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != pps_set_lamp_color(*(cyf->pps->fd->bbserial),cyf->chan,0x03))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->pps->fd->sockfd,cyf->pps->cs,cyf->chan,0x03,cyf->pps->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != pps_set_lamp_color(*(cyf->pps->fd->bbserial),cyf->chan,0x01))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->pps->fd->sockfd,cyf->pps->cs,cyf->chan,0x01,cyf->pps->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int pps_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,ppspinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(ppspinfo_t));
	pps_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
