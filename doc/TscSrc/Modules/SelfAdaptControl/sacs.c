...
int sa_get_phase_info(fcdata_t *fd,tscdata_t *td,phasedetect_t *pd,unsigned char tcline,unsigned char slnum,sapinfo_t *pinfo)
{
...
int sa_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int sa_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef SELF_ADAPT_DEBUG
...
int sa_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef SELF_ADAPT_DEBUG
...
int sa_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],sapinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef SELF_ADAPT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef SELF_ADAPT_DEBUG
				...
				if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef SELF_ADAPT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef SELF_ADAPT_DEBUG
			...
			if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef SELF_ADAPT_DEBUG
		...
		{
			if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef SELF_ADAPT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int sa_jump_stage_control(markdata_c *mdt,phasedetect_t *pd,unsigned char staid,sapinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef SELF_ADAPT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef SELF_ADAPT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != sa_get_phase_info(mdt->fcdata,mdt->tscdata,pd,*(mdt->rettl),num,pinfo))
			{
			#ifdef SELF_ADAPT_DEBUG
...
void sa_red_flash(void *arg)
{
	...
	if (0 == (rdt->sa->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->sa->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->sa->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->sa->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->sa->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->sa->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->sa->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->sa->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->sa->fd->specfunc) & 0x10) && (*(rdt->sa->fd->specfunc) & 0x20))
            {
                tchan = rdt->sa->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->sa->fd->specfunc) & 0x20) && (!(*(rdt->sa->fd->specfunc) & 0x10)))
            {
                tchan = rdt->sa->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->sa->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->sa->fd->markbit) & 0x1000)) && (!(*(rdt->sa->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->sa->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != sa_set_lamp_color(*(rdt->sa->fd->bbserial),cchan,0x03))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(rdt->sa->fd->sockfd,rdt->sa->cs,cchan,0x03,rdt->sa->fd->markbit))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != sa_set_lamp_color(*(rdt->sa->fd->bbserial),cchan,0x00))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(rdt->sa->fd->sockfd,rdt->sa->cs,cchan,0x00,rdt->sa->fd->markbit))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->sa->fd->markbit) & 0x1000)) && (!(*(rdt->sa->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->sa->fd->sockfd->csockfd);
    }
...
int sa_mobile_jp_control(markdata_c *mdt,unsigned char staid,sapinfo_t *pinfo,phasedetect_t *pd,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef SELF_ADAPT_DEBUG
...
int sa_mobile_direct_control(markdata_c *mdt,sapinfo_t *pinfo,phasedetect_t *pd,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef SELF_ADAPT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != sa_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int sa_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,phasedetect_t *pd,unsigned char tcline,sapinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(sapinfo_t));
	sa_get_phase_info(fd,td,pd,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
