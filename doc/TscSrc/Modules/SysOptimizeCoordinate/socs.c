...
int soc_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,socpinfo_t *pinfo)
{
...
int soc_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int soc_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
...
int soc_send_traffic_para(unsigned char dn,int *csockfd,optidata_t *odc,socphasedetect_t *spd)
{
	...
	unsigned int			maot = 0;
    unsigned char           fix_green_time = 0;
	...
	{//one detector
		...
		{//0
			...
			for (j = 0; j < 10; j++)
			{//1
				if (0 == spd[i].socdetectpro[j].deteid)
					break;
				if (spd[i].kf <= spd[i].socdetectpro[j].icarnum)
				{
					spd[i].kf = spd[i].socdetectpro[j].icarnum;
					spd[i].kd = spd[i].socdetectpro[j].deteid;
					spd[i].kff = spd[i].socdetectpro[j].fullflow;
				}
				if (maxocctime <= spd[i].socdetectpro[j].maxocctime)
				{
					maxocctime = spd[i].socdetectpro[j].maxocctime;
				}
				if (0 == spd[i].socdetectpro[j].icarnum)
					spd[i].socdetectpro[j].icarnum = 1;
	
				if (maot <= ((spd[i].socdetectpro[j].maxocctime)/(spd[i].socdetectpro[j].icarnum)))
					maot = (spd[i].socdetectpro[j].maxocctime)/(spd[i].socdetectpro[j].icarnum);
				spd[i].pf += spd[i].socdetectpro[j].icarnum;
				spd[i].pf += spd[i].socdetectpro[j].redflow; 

				spd[i].socdetectpro[j].icarnum = 0;
				spd[i].socdetectpro[j].ocarnum = 0;
				spd[i].socdetectpro[j].maxocctime = 0;
				spd[i].socdetectpro[j].redflow = 0;	
			}//1
			spd[i].maot = maot;
            spd[i].occrate =  \
			(double)maxocctime/(double)(1000*(spd[i].syscoofixgreen + spd[i].ytime - spd[i].startdelay));		
            spd[i].pt = spd[i].syscoofixgreen +  spd[i].ytime + spd[i].startdelay + spd[i].rtime;
			fenzi =spd[i].kf*(double)3600;
			fenmu = (double)spd[i].kff *(double)spd[i].syscoofixgreen; 
...
//				{
            ...
            if ((-0.000001 < spd[i].kf) && (spd[i].kf < 0.000001))
                spd[i].kf = 1;
			spd[i].chw = (spd[i].syscoofixgreen + spd[i].gftime)/spd[i].kf;
	...
	for(i = 0; i < MAX_PHASE_LINE; i++)
	{
		if (0 == spd[i].stageid)
			break;
		printf("pfd: %lf,kff: %d,pc:%lf,Line: %d\n",spd[i].pfd,spd[i].kff,spd[i].pc,__LINE__);
		printf("i:%d,phaseid:%d,phasetime:%d,phaseflow:%d,keyroadflow:%d,phasefulldegree:%lf,keydetector:%d, \
				keyflowratio:%lf,avecarspeed:%d,avecardelay:%lf,stopcartime:%lf,occrate:%lf,carheadway:%lf, \
				passcapacity:%lf,maot:%d\n",i,spd[i].phaseid,spd[i].pt,spd[i].pf,spd[i].kf,spd[i].pfd, \
...
int soc_backup_pattern_data(int serial,unsigned char snum,socphasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
...
int soc_dirch_control(markdata_t *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],socpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
				...
				if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			...
			if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
		...
		{
			if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int soc_jump_stage_control(markdata_t *mdt,unsigned char staid,socpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != soc_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
...
void soc_red_flash(void *arg)
{
	...
	if (0 == (rdt->soc->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->soc->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->soc->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->soc->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->soc->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->soc->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->soc->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->soc->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->soc->fd->specfunc) & 0x10) && (*(rdt->soc->fd->specfunc) & 0x20))
            {
                tchan = rdt->soc->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->soc->fd->specfunc) & 0x20) && (!(*(rdt->soc->fd->specfunc) & 0x10)))
            {
                tchan = rdt->soc->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->soc->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->soc->fd->markbit) & 0x1000)) && (!(*(rdt->soc->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->soc->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != soc_set_lamp_color(*(rdt->soc->fd->bbserial),cchan,0x03))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->soc->fd->sockfd,rdt->soc->cs,cchan,0x03,rdt->soc->fd->markbit))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != soc_set_lamp_color(*(rdt->soc->fd->bbserial),cchan,0x00))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->soc->fd->sockfd,rdt->soc->cs,cchan,0x00,rdt->soc->fd->markbit))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->soc->fd->markbit) & 0x1000)) && (!(*(rdt->soc->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->soc->fd->sockfd->csockfd);
    }	
...
int soc_mobile_jp_control(markdata_t *mdt,unsigned char staid,socpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
...
int soc_mobile_direct_control(markdata_t *mdt,socpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != soc_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef SYSTEM_OPTIMIZE_COORD_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int soc_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,socpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(socpinfo_t));
	soc_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
