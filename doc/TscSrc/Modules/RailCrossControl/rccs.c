...
int rcc_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,rccpinfo_t *pinfo)
{
	...
	{//0000000000
		...
		if (phaseid == (td->channel->ChannelList[i].ChannelCtrlSrc))
		{
			...
			{
				pinfo->pchan[m] = td->channel->ChannelList[i].ChannelId;
				m++;
				pinfo->pcexist = 1;
			}
			else
			{
				pinfo->npchan[n] = td->channel->ChannelList[i].ChannelId;
				n++;
			}
			exist = 0;
...
int rcc_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int rcc_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef RCC_DEBUG
...
int rcc_jump_stage_control(markdata_c *mdt,unsigned char staid,rccpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef RCC_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef RCC_DEBUG
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != rcc_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef RCC_DEBUG
	...
	{//other condition
		...
		{//Controlling stageid is not current stageid
			...
			while (1)
			{//green flash
				if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef RCC_DEBUG
				...
				if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
                {
                #ifdef RCC_DEBUG
                    printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
                #endif
			...
			if (1 == pcexist)
			{
				...
				if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
				{
				#ifdef RCC_DEBUG
				...
				if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
				{
				#ifdef RCC_DEBUG
			...
			{
				if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
				{
				#ifdef RCC_DEBUG
					printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
			...
			if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
			{
			#ifdef RCC_DEBUG
				printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
...
int rcc_mobile_jp_control(markdata_t *mdt,unsigned char staid,rccpinfo_t *pinfo,unsigned char *dirch)
{
...
int rcc_mobile_direct_control(markdata_t *mdt,rccpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef RCC_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != rcc_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef RCC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int rcc_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,rccpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(rccpinfo_t));
	rcc_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
