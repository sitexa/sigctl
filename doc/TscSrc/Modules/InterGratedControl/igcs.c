...
int igc_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],InterGrateControlPhaseInfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef FULL_DETECT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef FULL_DETECT_DEBUG
				...
				if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef FULL_DETECT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef FULL_DETECT_DEBUG
			...
			if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef FULL_DETECT_DEBUG
		...
		{
			if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef FULL_DETECT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef FULL_DETECT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int igc_jump_stage_control(markdata_c *mdt,unsigned char staid,InterGrateControlPhaseInfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef FULL_DETECT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		#endif
			memset(pinfo,0,sizeof(InterGrateControlPhaseInfo_t));
			if (SUCCESS != igc_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef FULL_DETECT_DEBUG
...
int igc_mobile_direct_control(markdata_c *mdt,InterGrateControlPhaseInfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != igc_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef FULL_DETECT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
void igc_channel_yellow_flash(InterGrateControlData_t *stInterGrateControlData)
{
	...
	unsigned char				num = 0;
	unsigned char               acYellowFlashChane[MAX_CHANNEL] = {0};
	struct timeval          to;
	...
	for (i = 0; i < MAX_CHANNEL; i++)
	{
		acYellowFlashChane[i] = i+1;
	}
	printf("start to  yellow flash\n");
	if ((!(*(stInterGrateControlData->fd->markbit) & 0x1000)) && (!(*(stInterGrateControlData->fd->markbit) & 0x8000)))
	{
		...
		stSinfo.chans = 0;
		stSinfo.color = 5;
		stSinfo.pattern = *(stInterGrateControlData->fd->patternid);
		stSinfo.cyclet = *(stInterGrateControlData->fd->cyclet);
		stSinfo.stage = *(stInterGrateControlData->fd->stageid);
        stSinfo.phase = *(stInterGrateControlData->fd->phaseid);
	...
	while (1)
	{//while
		if (SUCCESS != igc_set_lamp_color(*(stInterGrateControlData->fd->bbserial),acYellowFlashChane,0x03))
		{
			#ifdef INTER_CONTROL_DEBUG
				printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		}		
		if (SUCCESS != update_channel_status(stInterGrateControlData->fd->sockfd,stInterGrateControlData->cs,acYellowFlashChane,0x03,stInterGrateControlData->fd->markbit))
		{
			#ifdef INTER_CONTROL_DEBUG
				printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != igc_set_lamp_color(*(stInterGrateControlData->fd->bbserial),acYellowFlashChane,0x01))
		{
			#ifdef INTER_CONTROL_DEBUG
				printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		}		
		if (SUCCESS != update_channel_status(stInterGrateControlData->fd->sockfd,stInterGrateControlData->cs,acYellowFlashChane,0x01,stInterGrateControlData->fd->markbit))
		{
			#ifdef INTER_CONTROL_DEBUG
				printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
...
int igc_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,InterGrateControlPhaseInfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(InterGrateControlPhaseInfo_t));
	igc_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
