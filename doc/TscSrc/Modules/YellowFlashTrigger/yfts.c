...
int yft_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,yftpinfo_t *pinfo)
{
...
int yft_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int yft_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef YFT_DEBUG
...
int yft_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],yftpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef YFT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef YFT_DEBUG
				...
				if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef YFT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef YFT_DEBUG
			...
			if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef YFT_DEBUG
		...
		{
			if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef YFT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef YFT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int yft_jump_stage_control(markdata_c *mdt,unsigned char staid,yftpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef YFT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef YFT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != yft_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef YFT_DEBUG
...
int yft_mobile_jp_control(markdata_c *mdt,unsigned char staid,yftpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef YFT_DEBUG
...
int yft_mobile_direct_control(markdata_c *mdt,yftpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef YFT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != yft_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef YFT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int yft_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,yftpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(yftpinfo_t));
	yft_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
