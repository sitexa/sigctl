...
int madc_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,mahpinfo_t *pinfo)
{
...
int madc_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int madc_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef MAJOR_DEBUG
...
int mah_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef MAJOR_DEBUG
...
int madc_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],mahpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef MAJOR_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef MAJOR_DEBUG
				...
				if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef MAJOR_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef MAJOR_DEBUG
			...
			if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef MAJOR_DEBUG
		...
		{
			if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef MAJOR_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef MAJOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int madc_jump_stage_control(markdata_c *mdt,unsigned char staid,mahpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef MAJOR_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef MAJOR_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != madc_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef MAJOR_DEBUG
...
void mah_red_flash(void *arg)
{
	...
	if (0 == (rdt->mah->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->mah->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->mah->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->mah->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->mah->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->mah->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->mah->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->mah->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->mah->fd->specfunc) & 0x10) && (*(rdt->mah->fd->specfunc) & 0x20))
            {
                tchan = rdt->mah->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->mah->fd->specfunc) & 0x20) && (!(*(rdt->mah->fd->specfunc) & 0x10)))
            {
                tchan = rdt->mah->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->mah->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->mah->fd->markbit) & 0x1000)) && (!(*(rdt->mah->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->mah->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != madc_set_lamp_color(*(rdt->mah->fd->bbserial),cchan,0x03))
		{
		#ifdef MAJOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->mah->fd->sockfd,rdt->mah->cs,cchan,0x03,rdt->mah->fd->markbit))
		{
		#ifdef MAJOR_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != madc_set_lamp_color(*(rdt->mah->fd->bbserial),cchan,0x00))
		{
		#ifdef MAJOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->mah->fd->sockfd,rdt->mah->cs,cchan,0x00,rdt->mah->fd->markbit))
		{
		#ifdef MAJOR_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->mah->fd->markbit) & 0x1000)) && (!(*(rdt->mah->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->mah->fd->sockfd->csockfd);
    }
...
int madc_mobile_jp_control(markdata_c *mdt,unsigned char staid,mahpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef MAJOR_DEBUG
...
int madc_mobile_direct_control(markdata_c *mdt,mahpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef MAJOR_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != madc_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef MAJOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
void mah_channel_yellow_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != madc_set_lamp_color(*(cyf->mah->fd->bbserial),cyf->chan,0x03))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->mah->fd->sockfd,cyf->mah->cs,cyf->chan,0x03,cyf->mah->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != madc_set_lamp_color(*(cyf->mah->fd->bbserial),cyf->chan,0x01))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->mah->fd->sockfd,cyf->mah->cs,cyf->chan,0x01,cyf->mah->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int madc_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,mahpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(mahpinfo_t));
	madc_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
