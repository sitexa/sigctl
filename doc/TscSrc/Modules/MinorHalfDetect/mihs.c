...
int mih_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,mihpinfo_t *pinfo)
{
...
int mih_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int mih_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef MINOR_DEBUG
...
int mih_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef MINOR_DEBUG
...
int mih_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],mihpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef MINOR_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef MINOR_DEBUG
				...
				if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef MINOR_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef MINOR_DEBUG
			...
			if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef MINOR_DEBUG
		...
		{
			if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef MINOR_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef MINOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int mih_jump_stage_control(markdata_c *mdt,unsigned char staid,mihpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef MINOR_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef MINOR_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != mih_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef MINOR_DEBUG
...
#ifdef RED_FLASH
void mih_red_flash(void *arg)
{
	redflash_mih			*rdt = arg;	
	...
	if (0 == (rdt->mih->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->mih->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->mih->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->mih->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->mih->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->mih->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->mih->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->mih->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->mih->fd->specfunc) & 0x10) && (*(rdt->mih->fd->specfunc) & 0x20))
            {
                tchan = rdt->mih->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->mih->fd->specfunc) & 0x20) && (!(*(rdt->mih->fd->specfunc) & 0x10)))
            {
                tchan = rdt->mih->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->mih->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->mih->fd->markbit) & 0x1000)) && (!(*(rdt->mih->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->mih->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != mih_set_lamp_color(*(rdt->mih->fd->bbserial),cchan,0x03))
		{
		#ifdef MINOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->mih->fd->sockfd,rdt->mih->cs,cchan,0x03,rdt->mih->fd->markbit))
		{
		#ifdef MINOR_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != mih_set_lamp_color(*(rdt->mih->fd->bbserial),cchan,0x00))
		{
		#ifdef MINOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->mih->fd->sockfd,rdt->mih->cs,cchan,0x00,rdt->mih->fd->markbit))
		{
		#ifdef MINOR_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->mih->fd->markbit) & 0x1000)) && (!(*(rdt->mih->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->mih->fd->sockfd->csockfd);
    }
...
int mih_mobile_jp_control(markdata_c *mdt,unsigned char staid,mihpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef MINOR_DEBUG
...
int mih_mobile_direct_control(markdata_c *mdt,mihpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef MINOR_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != mih_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef MINOR_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
void mih_channel_yellow_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != mih_set_lamp_color(*(cyf->mih->fd->bbserial),cyf->chan,0x03))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->mih->fd->sockfd,cyf->mih->cs,cyf->chan,0x03,cyf->mih->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != mih_set_lamp_color(*(cyf->mih->fd->bbserial),cyf->chan,0x01))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->mih->fd->sockfd,cyf->mih->cs,cyf->chan,0x01,cyf->mih->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int mih_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,mihpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(mihpinfo_t));
	mih_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
