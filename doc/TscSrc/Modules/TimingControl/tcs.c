...
int tc_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,phaseinfo_t *pinfo)
{
...
int tc_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int tc_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef TIMING_DEBUG
...
int tc_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef TIMING_DEBUG
...
int tc_dirch_control(markdata_t *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],phaseinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef TIMING_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef TIMING_DEBUG
				...
				if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef TIMING_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef TIMING_DEBUG
			...
			if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef TIMING_DEBUG
		...
		{
			if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef TIMING_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int tc_jump_stage_control(markdata_t *mdt,unsigned char staid,phaseinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef TIMING_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef TIMING_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != tc_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef TIMING_DEBUG
...
void tc_channel_yellow_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != tc_set_lamp_color(*(cyf->tc->fd->bbserial),cyf->chan,0x03))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->tc->fd->sockfd,cyf->tc->cs,cyf->chan, \
						0x03,cyf->tc->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != tc_set_lamp_color(*(cyf->tc->fd->bbserial),cyf->chan,0x01))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
#if 0	
	if (SUCCESS != tc_set_lamp_color(*(cyf->tc->fd->bbserial),cyf->chan,0x00))
	{
	#ifdef TIMING_DEBUG
		printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
	#endif
...
void tc_red_flash(void *arg)
{
	...
	if (0 == (rdt->tc->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->tc->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->tc->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->tc->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->tc->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->tc->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
			tchan = rdt->tc->td->channel->ChannelList[i].ChannelId;
			if ((tchan >= 0x05) && (tchan <= 0x0c))
			{
				if (*(rdt->tc->fd->specfunc) & (0x01 << (tchan - 5)))
					continue;	
			}
		#else
			if ((*(rdt->tc->fd->specfunc) & 0x10) && (*(rdt->tc->fd->specfunc) & 0x20))
            {
                tchan = rdt->tc->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
			...
			if ((*(rdt->tc->fd->specfunc) & 0x20) && (!(*(rdt->tc->fd->specfunc) & 0x10)))
			{
				tchan = rdt->tc->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
		...
		#endif
			nchan[j] = rdt->tc->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->tc->fd->markbit) & 0x1000)) && (!(*(rdt->tc->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->tc->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != tc_set_lamp_color(*(rdt->tc->fd->bbserial),cchan,0x03))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(rdt->tc->fd->sockfd,rdt->tc->cs,cchan,0x03,rdt->tc->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != tc_set_lamp_color(*(rdt->tc->fd->bbserial),cchan,0x00))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(rdt->tc->fd->sockfd,rdt->tc->cs,cchan,0x00,rdt->tc->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->tc->fd->markbit) & 0x1000)) && (!(*(rdt->tc->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->tc->fd->sockfd->csockfd);
    }	
...
int tc_mobile_jp_control(markdata_t *mdt,unsigned char staid,phaseinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef TIMING_DEBUG
...
int tc_mobile_direct_control(markdata_t *mdt,phaseinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef TIMING_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != tc_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
