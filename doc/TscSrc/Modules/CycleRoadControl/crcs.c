...
int crc_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,dcpinfo_t *pinfo)
{
...
int crc_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int crc_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef CRC_DEBUG
...
int crc_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef CRC_DEBUG
...
int crc_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],dcpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef CRC_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef CRC_DEBUG
				...
				if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef CRC_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef CRC_DEBUG
			...
			if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef CRC_DEBUG
		...
		{
			if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef CRC_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef CRC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int crc_jump_stage_control(markdata_c *mdt,unsigned char staid,dcpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef CRC_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef CRC_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != crc_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef CRC_DEBUG
...
void crc_red_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != crc_set_lamp_color(*(rdt->dc->fd->bbserial),cchan,0x03))
		{
		#ifdef CRC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != crc_set_lamp_color(*(rdt->dc->fd->bbserial),cchan,0x00))
		{
		#ifdef CRC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int crc_mobile_jp_control(markdata_c *mdt,unsigned char staid,dcpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef CRC_DEBUG
...
int crc_mobile_direct_control(markdata_c *mdt,dcpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef CRC_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != crc_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef CRC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
void crc_channel_yellow_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != crc_set_lamp_color(*(cyf->dc->fd->bbserial),cyf->chan,0x03))
		{
		#ifdef CRC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != crc_set_lamp_color(*(cyf->dc->fd->bbserial),cyf->chan,0x01))
		{
		#ifdef CRC_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int crc_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,dcpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(dcpinfo_t));
	crc_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
