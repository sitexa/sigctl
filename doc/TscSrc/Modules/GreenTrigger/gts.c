...
int gt_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,gtpinfo_t *pinfo)
{
...
int gt_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int gt_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef GT_DEBUG
...
int gt_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],gtpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef GT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef GT_DEBUG
				...
				if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef GT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef GT_DEBUG
			...
			if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef GT_DEBUG
		...
		{
			if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef GT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef GT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int gt_jump_stage_control(markdata_c *mdt,unsigned char staid,gtpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef GT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef GT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != gt_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef GT_DEBUG
...
int gt_mobile_jp_control(markdata_c *mdt,unsigned char staid,gtpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef GT_DEBUG
...
int gt_mobile_direct_control(markdata_c *mdt,gtpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef GT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != gt_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef GT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int gt_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,gtpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(gtpinfo_t));
	gt_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
