					...
					{//1
						...
						{//err yellow flash
							...
							if ((markbit & 0x0002) && (markbit & 0x0010))
							{
								...
								if (SUCCESS != timedown_data_to_conftool(ssockfd,&timedown))
								{
								#ifdef MAIN_DEBUG
...
//										mc_combine_str(sdata,"CYT4",total,buf,len,"END");
									...
									if (0x88 == objecti[0].objectid)
									{//time requery,keli system requery time
										gettimeofday(&tt,NULL);
									#ifdef MAIN_DEBUG
										printf("now time: %d,File:%s,LIne:%d\n",tt.tv_sec,__FILE__,__LINE__);
									#endif
										memset(vibuf,0,sizeof(vibuf));
										if (SUCCESS != time_info_report_keli(vibuf,tt.tv_sec))
                                        {
                                        #ifdef MAIN_DEBUG
                                            printf("version id pack err,File:%s,Line:%d\n",__FILE__,__LINE__);
										#endif
									...
									{//ID code and version requery;
										...
										unsigned char			bufn[40] = {0};
										unsigned char			bufp[40] = {0};
										unsigned char			*pbuf = bufp;
										FILE 					*pzfp = NULL;
										unsigned char			pzi = 0;

										pzfp = fopen(MACHINE_VER,"r");
										if (NULL == pzfp)
										{
										#ifdef MAIN_DEBUG
										...
										pzi = 0;
										while (NULL != fgets(buf,sizeof(buf),pzfp))
										{
											if (0 == pzi)
											{
												strncpy(bufn,buf,16);
								...
								{//set request or no response;
									if (0 == itype.objectn)
									{
										...
										else if (0x88 == objecti[0].objectid)
										{//set time,keli system set time;
...
*/
	...
	for (di = 0; di < detector.FactDetectorNum; di++)
	{
        ...
        tnum >>= 2;
        tnum &= 0x3f;
        detepro[di].intime = tnum * 60; //seconds;
		gettimeofday(&detepro[di].stime,NULL);	//added by sk on 20150625	
	}
	//end add
...
*/
	...
	pthread_t			serid;
	int se = pthread_create(&serid,NULL,(void *)monitor_serial_port,NULL);
	if (0 != se)
	{
	#ifdef MAIN_DEBUG
		printf("create thread error,File: %s,Line: %d\n",__FILE__,__LINE__);
