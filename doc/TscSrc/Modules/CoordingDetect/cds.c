...
int cd_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,cdpinfo_t *pinfo)
{
...
int cd_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int cd_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef COORD_DETECT_DEBUG
...
int cd_backup_pattern_data(int serial,unsigned char snum,cdphasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef COORD_DETECT_DEBUG
...
int cd_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],cdpinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef COORD_DETECT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef COORD_DETECT_DEBUG
				...
				if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef COORD_DETECT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef COORD_DETECT_DEBUG
			...
			if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef COORD_DETECT_DEBUG
		...
		{
			if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef COORD_DETECT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef COORD_DETECT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int cd_jump_stage_control(markdata_c *mdt,unsigned char staid,cdpinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef COORD_DETECT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef COORD_DETECT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != cd_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef COORD_DETECT_DEBUG
...
void cd_red_flash(void *arg)
{
	...
	if (0 == (rdt->cd->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->cd->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->cd->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->cd->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->cd->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->cd->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->cd->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->cd->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->cd->fd->specfunc) & 0x10) && (*(rdt->cd->fd->specfunc) & 0x20))
            {
                tchan = rdt->cd->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->cd->fd->specfunc) & 0x20) && (!(*(rdt->cd->fd->specfunc) & 0x10)))
            {
                tchan = rdt->cd->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->cd->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->cd->fd->markbit) & 0x1000)) && (!(*(rdt->cd->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->cd->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != cd_set_lamp_color(*(rdt->cd->fd->bbserial),cchan,0x03))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(rdt->cd->fd->sockfd,rdt->cd->cs,cchan,0x03,rdt->cd->fd->markbit))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != cd_set_lamp_color(*(rdt->cd->fd->bbserial),cchan,0x00))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(rdt->cd->fd->sockfd,rdt->cd->cs,cchan,0x00,rdt->cd->fd->markbit))
		{
		#ifdef SELF_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->cd->fd->markbit) & 0x1000)) && (!(*(rdt->cd->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->cd->fd->sockfd->csockfd);
    }
...
int cd_mobile_jp_control(markdata_c *mdt,unsigned char staid,cdpinfo_t *pinfo,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef COORD_DETECT_DEBUG
...
int cd_mobile_direct_control(markdata_c *mdt,cdpinfo_t *pinfo,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef COORD_DETECT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != cd_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef COORD_DETECT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int cd_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,cdpinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(cdpinfo_t));
	cd_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
