...
int soa_get_phase_info(fcdata_t *fd,tscdata_t *td,soaphasedetect_t *pd,unsigned char tcline,unsigned char slnum,soapinfo_t *pinfo)
{
	if ((NULL == td) || (NULL == pinfo) || (NULL == pd))
	{
	...
	for (i = 0; i < MAX_PHASE_LINE; i++)
	{
		if (0 == (pd->phaseid))
			break;
		if (phaseid == (pd->phaseid))
		{
			pinfo->fcgtime = pd->fcgtime;
			pinfo->ytime = pd->ytime;
			pinfo->rtime = pd->rtime;
			pinfo->gftime = pd->gftime;
			pinfo->phasetype = pd->phasetype;
			pinfo->mingtime = pd->mingtime;
			pinfo->maxgtime1 = pd->maxgtime;
			pinfo->fixgtime = pd->fixgtime;
			pinfo->pgtime = pd->pgtime;
			pinfo->pctime = pd->pctime;
			pinfo->gtime = pd->gtime;
			break;
		}
		pd++;
	}
...
int soa_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int soa_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
...
int soa_send_traffic_para(unsigned char dn,int *csockfd,optidata_t *od,soaphasedetect_t *spd)
{
	if ((NULL == od) || (NULL == spd) || (NULL == csockfd))
	{
	#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
	...
	double					temp4 = 0;
	double					temp5 = 0;
	unsigned char			*reflags = od->reflags;
	unsigned long			maxocctime = 0;
	unsigned int			maot = 0;
	unsigned char			tgt = 0;

	if ((od->opticycle) > 0)           //the cycle is not current cycle, the actually cycle is last
	{
		od->cycle = od->opticycle;
	} 
	...
	{//one detector
		...
		{//0
			...
			for (j = 0; j < 10; j++)
			{//1
				if (0 == spd[i].soadetectpro[j].deteid)
					break;
				if (spd[i].kf <= spd[i].soadetectpro[j].icarnum)
				{
					spd[i].kf = spd[i].soadetectpro[j].icarnum;
					spd[i].kd = spd[i].soadetectpro[j].deteid;
					spd[i].kff = spd[i].soadetectpro[j].fullflow;
				}
				if (maxocctime <= spd[i].soadetectpro[j].maxocctime)
				{
					maxocctime = spd[i].soadetectpro[j].maxocctime;
				}
				
//				if (0 == spd[i].soadetectpro[j].icarnum)
//					spd[i].soadetectpro[j].icarnum = 1;	
                if(0 != spd[i].soadetectpro[j].icarnum)
                {
                    if (maot <= ((spd[i].soadetectpro[j].maxocctime)/(spd[i].soadetectpro[j].icarnum)))
                        maot = (spd[i].soadetectpro[j].maxocctime)/(spd[i].soadetectpro[j].icarnum);     
                }
                spd[i].pf += spd[i].soadetectpro[j].icarnum;
                spd[i].pf += spd[i].soadetectpro[j].redflow;

				spd[i].soadetectpro[j].icarnum = 0;
				spd[i].soadetectpro[j].ocarnum = 0;
				spd[i].soadetectpro[j].maxocctime = 0;
				spd[i].soadetectpro[j].redflow = 0;	
			}//1
...
/*			if ((spd[i].maot >= 5000) && (spd[i].maot <= 20000))
            {
                tgt = 60;
            }
			if (spd[i].gtime < tgt)
...
//			od->opticycle += spd[i].pt;
			od->opticycle += spd[i].optipt;
		}//0
		for (i = 0; i < MAX_PHASE_LINE; i++)
        {//1
            if (0 == spd[i].stageid)
			...
			if ((-0.000001 < (od->cycle)) && ((od->cycle) < 0.000001))
                od->cycle = 1;	
			gratio = (spd[i].gtbak)/(od->cycle);
			if (spd[i].pfd < 1)
			{
				fenzi = (od->cycle) * (1-gratio) * (1-gratio);
				...
				temp4 = (double)spd[i].kf * spd[i].kf;
				if ((-0.000001 < temp4) && (temp4 < 0.000001))
                    temp4 = 1;
				temp4 = (od->cycle)/temp4;
			...
			{//level 1 road
//				temp4 = (spd[i].kf*((double)3600/od->cycle))/(double)1560;
...
//				}
			...
			{//level 2 road
//				temp4 = (spd[i].kf*((double)3600/od->cycle))/(double)1450;
...
//				{
			...
			fenzi = (od->cycle) - (spd[i].gtbak + spd[i].gftime);
			fenmu = (od->cycle)*(1 - (spd[i].kfr));
			if ((-0.000001 < fenmu) && (fenmu < 0.000001))
            	fenmu = 1;
	...
	printf("patternid: %d,cycle: %d,phasenum: %d,opticycle: %d\n", \
				od->patternid,od->cycle,od->phasenum,od->opticycle);
	for(i = 0; i < MAX_PHASE_LINE; i++)
	{
		if (0 == spd[i].stageid)
			break;
...
int soa_backup_pattern_data(int serial,unsigned char snum,soaphasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
...
int soa_send_traffic_para(unsigned char dn,int *csockfd,optidata_t *od,soaphasedetect_t *spd)
{
	...
	{//one detector
		...
		{//0
			...
			od->phasenum += 1;
			spd[i].pt = 0;//phase time
			spd[i].optipt = 0;//optimize phase time
			spd[i].pf = 0; //phase flow
			spd[i].kf = 0; //key road flow
			spd[i].kff = 0; //key road full flow
			spd[i].pfd = 0;//phase full degree
...
int soa_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],soapinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
				...
				if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			...
			if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
		...
		{
			if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int soa_jump_stage_control(markdata_c *mdt,soaphasedetect_t *pd,unsigned char staid,soapinfo_t *pinfo)
{
	if (NULL == mdt)
	{
	#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != soa_get_phase_info(mdt->fcdata,mdt->tscdata,pd,*(mdt->rettl),num,pinfo))
			{
			#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
...
void soa_red_flash(void *arg)
{
	...
	if (0 == (rdt->soa->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->soa->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->soa->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->soa->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->soa->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->soa->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->soa->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->soa->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->soa->fd->specfunc) & 0x10) && (*(rdt->soa->fd->specfunc) & 0x20))
            {
                tchan = rdt->soa->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->soa->fd->specfunc) & 0x20) && (!(*(rdt->soa->fd->specfunc) & 0x10)))
            {
                tchan = rdt->soa->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->soa->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->soa->fd->markbit) & 0x1000)) && (!(*(rdt->soa->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->soa->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != soa_set_lamp_color(*(rdt->soa->fd->bbserial),cchan,0x03))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->soa->fd->sockfd,rdt->soa->cs,cchan,0x03,rdt->soa->fd->markbit))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != soa_set_lamp_color(*(rdt->soa->fd->bbserial),cchan,0x00))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS!=update_channel_status(rdt->soa->fd->sockfd,rdt->soa->cs,cchan,0x00,rdt->soa->fd->markbit))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->soa->fd->markbit) & 0x1000)) && (!(*(rdt->soa->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->soa->fd->sockfd->csockfd);
    }

	pthread_detach(pthread_self());//added by shikang on 20161018
	pthread_exit(NULL);
}
#endif

//mobile achieve jump phase control
int soa_mobile_jp_control(markdata_c *mdt,unsigned char staid,soapinfo_t *pinfo,soaphasedetect_t *pd,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
...
int soa_mobile_direct_control(markdata_c *mdt,soapinfo_t *pinfo,soaphasedetect_t *pd,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != soa_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef SYSTEM_OPTIMIZE_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int soa_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,soaphasedetect_t *pd,unsigned char tcline,soapinfo_t *pinfo)
{
	if((NULL == fd) || (td == NULL)|| (pinfo == NULL)||(NULL == pd))
	{
	...
	memset(pinfo,0,sizeof(soapinfo_t));
	soa_get_phase_info(fd,td,pd,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
