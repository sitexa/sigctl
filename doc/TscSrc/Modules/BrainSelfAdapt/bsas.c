...
int bsa_get_phase_info(fcdata_t *fd,tscdata_t *td,unsigned char tcline,unsigned char slnum,bsapinfo_t *pinfo)
{
...
/*
int bsa_get_phase_info(fcdata_t *fd,tscdata_t *td,phasedetect_t *pd,unsigned char tcline,unsigned char slnum,bsapinfo_t *pinfo)
{
	if ((NULL == td) || (NULL == pinfo) || (NULL == pd) || (NULL == fd))
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
		printf("pointer address is null,File: %s,Line: %d\n",__FILE__,__LINE__);
	#endif
...
*/
int bsa_get_timeconfig(tscdata_t *td,unsigned char *pattern,unsigned char *tcline)
{
...
int bsa_set_lamp_color(int serial,unsigned char *chan,unsigned char color)
{
	if (NULL == chan)
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
...
int bsa_backup_pattern_data(int serial,unsigned char snum,phasedetect_t *phd)
{
	if ((NULL == phd) || (0 == snum))
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
...
int bsa_dirch_control(markdata_c *mdt,unsigned char cktem,unsigned char ktem,unsigned char (*dirch)[8],bsapinfo_t *pinfo)
{
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),dirch[ktem-0x20],0x02))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
	...
	{//other condition
		...
		if ((0 != cchan[0]) && (pinfo->gftime > 0))
		{
			while (1)
			{//green flash
				if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x03))
				{
				#ifdef BRAIN_SELF_ADAPT_DEBUG
				...
				if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x02))
				{
				#ifdef BRAIN_SELF_ADAPT_DEBUG
					printf("close lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
				#endif
		...
		if (1 == pcexist)
		{
			...
			if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),cnpchan,0x01))
			{
			#ifdef BRAIN_SELF_ADAPT_DEBUG
			...
			if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),cpchan,0x00))
			{
			#ifdef BRAIN_SELF_ADAPT_DEBUG
		...
		{
			if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x01))
			{
			#ifdef BRAIN_SELF_ADAPT_DEBUG
				printf("set yellow lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
			#endif
		...
		if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),cchan,0x00))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int bsa_jump_stage_control(markdata_c *mdt,phasedetect_t *pd,unsigned char staid,bsapinfo_t *pinfo)
{
	if ((NULL == mdt) || (NULL == pd) ||(NULL == pinfo))
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
		printf("pointer address error,File: %s,Line: %d\n",__FILE__,__LINE__);
	#endif
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		#endif
			if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),pinfo->chan,0x02))
            {
            #ifdef BRAIN_SELF_ADAPT_DEBUG
                printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
            #endif
		...
		{//Controlling stageid is not current stageid;
			...
			if (SUCCESS != bsa_get_phase_info(mdt->fcdata,mdt->tscdata,*(mdt->rettl),num,pinfo))
			{
			#ifdef BRAIN_SELF_ADAPT_DEBUG
...
void bsa_red_flash(void *arg)
{
	...
	if (0 == (rdt->bsa->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].StageId))
	{
		get_phase_id(rdt->bsa->td->timeconfig->TimeConfigList[rdt->tcline][0].PhaseId,&nphaseid);
	}
	else
	{
		get_phase_id(rdt->bsa->td->timeconfig->TimeConfigList[rdt->tcline][rdt->slnum+1].PhaseId,&nphaseid);
	}
	

	for (i = 0; i < (rdt->bsa->td->channel->FactChannelNum); i++)
	{
		if (0 == (rdt->bsa->td->channel->ChannelList[i].ChannelId))
			break;
		if (nphaseid == (rdt->bsa->td->channel->ChannelList[i].ChannelCtrlSrc))
		{
		#ifdef CLOSE_LAMP
            tchan = rdt->bsa->td->channel->ChannelList[i].ChannelId;
            if ((tchan >= 0x05) && (tchan <= 0x0c))
            {
                if (*(rdt->bsa->fd->specfunc) & (0x01 << (tchan - 5)))
                    continue;
            }
        #else
            if ((*(rdt->bsa->fd->specfunc) & 0x10) && (*(rdt->bsa->fd->specfunc) & 0x20))
            {
                tchan = rdt->bsa->td->channel->ChannelList[i].ChannelId;
                if (((5 <= tchan) && (tchan <= 8)) || ((9 <= tchan) && (tchan <= 12)))
                    continue;
            ...
            if ((*(rdt->bsa->fd->specfunc) & 0x20) && (!(*(rdt->bsa->fd->specfunc) & 0x10)))
            {
                tchan = rdt->bsa->td->channel->ChannelList[i].ChannelId;
                if ((9 <= tchan) && (tchan <= 12))
                    continue;
        ...
        #endif
			nchan[j] = rdt->bsa->td->channel->ChannelList[i].ChannelId;
			j++;
		}
	}
	...
	//begin to red flash
	if ( (!(*(rdt->bsa->fd->markbit) & 0x1000)) && (!(*(rdt->bsa->fd->markbit) & 0x8000)) )
    {
        red_flash_begin_report(rdt->bsa->fd->sockfd->csockfd,cchan);
    }
	while (1)
	{//while
		if (SUCCESS != bsa_set_lamp_color(*(rdt->bsa->fd->bbserial),cchan,0x03))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if(SUCCESS!=update_channel_status(rdt->bsa->fd->sockfd,rdt->bsa->cs,cchan,0x03,rdt->bsa->fd->markbit))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != bsa_set_lamp_color(*(rdt->bsa->fd->bbserial),cchan,0x00))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if(SUCCESS!=update_channel_status(rdt->bsa->fd->sockfd,rdt->bsa->cs,cchan,0x00,rdt->bsa->fd->markbit))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
	...
	if ( (!(*(rdt->bsa->fd->markbit) & 0x1000)) && (!(*(rdt->bsa->fd->markbit) & 0x8000)) )
    {
        red_flash_end_report(rdt->bsa->fd->sockfd->csockfd);
    }
...
int bsa_mobile_jp_control(markdata_c *mdt,unsigned char staid,bsapinfo_t *pinfo,phasedetect_t *pd,unsigned char *dirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch) || (NULL == pd))
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
		printf("pointer address error,File: %s,Line: %d\n",__FILE__,__LINE__);
	#endif
...
int bsa_mobile_direct_control(markdata_c *mdt,bsapinfo_t *pinfo,phasedetect_t *pd,unsigned char *dirch,unsigned char *fdirch)
{
	if ((NULL == mdt) || (NULL == pinfo) || (NULL == dirch))
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
	...
	{//all red or all close or yellow flash
		...
		*(mdt->redl) = 0;	
		...
		if (SUCCESS != bsa_set_lamp_color(*(mdt->fcdata->bbserial),dirch,0x02))
		{
		#ifdef BRAIN_SELF_ADAPT_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int bsa_clean_phase_queue(unsigned char phaseid,phasedetect_t *pd)
{
	if (NULL == pd)
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
...
int bsa_get_phase_queue(phasedetect_t *pd,QueueDataList_t *qdl)
{
	if ((NULL == pd) || (NULL == qdl))
	{
	#ifdef BRAIN_SELF_ADAPT_DEBUG
	...
	for (i = 0; i < MAX_PHASE_LINE; i++)
	{
		...
		for (j = 0; j < DETN; j++)
		{
			if (0 == pd[i].detectn[j])	
				break;
			if (1 == qdl->QueueData[pd[i].detectn[j]-1].valid)
			{
			#ifdef BRAIN_SELF_ADAPT_DEBUG
				printf("phaseid: %d,detectid: %d,queuelen: %d,queuenum: %d,File: %s,Line: %d\n", \
						pd[i].phaseid,pd[i].detectn[j],qdl->QueueData[pd[i].detectn[j]-1].QueueLen, \
						qdl->QueueData[pd[i].detectn[j]-1].QueueNum,__FILE__,__LINE__);
			#endif
//				qdl->QueueData[pd[i].detectn[j]-1].valid = 0;
				if (pd[i].mquel < qdl->QueueData[pd[i].detectn[j]-1].QueueLen)
					pd[i].mquel = qdl->QueueData[pd[i].detectn[j]-1].QueueLen;
				if (pd[i].mquen < qdl->QueueData[pd[i].detectn[j]-1].QueueNum)
					pd[i].mquen = qdl->QueueData[pd[i].detectn[j]-1].QueueNum;
			}
		}
	}

	//Since multi-phase may share detects,so need to clean 0 of valid;
	for (i = 0; i < MAX_PHASE_LINE; i++)
    {               
        if (0 == pd[i].phaseid)
            break;
...
void bsa_channel_yellow_flash(void *arg)
{
	...
	while (1)
	{//while
		if (SUCCESS != bsa_set_lamp_color(*(cyf->bsa->fd->bbserial),cyf->chan,0x03))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->bsa->fd->sockfd,cyf->bsa->cs,cyf->chan,0x03,cyf->bsa->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		...
		if (SUCCESS != bsa_set_lamp_color(*(cyf->bsa->fd->bbserial),cyf->chan,0x01))
		{
		#ifdef TIMING_DEBUG
			printf("set green lamp err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
		}		
		if (SUCCESS != update_channel_status(cyf->bsa->fd->sockfd,cyf->bsa->cs,cyf->chan,0x01,cyf->bsa->fd->markbit))
		{
		#ifdef TIMING_DEBUG
			printf("update channel err,File: %s,Line: %d\n",__FILE__,__LINE__);
		#endif
...
int bsa_get_last_phaseinfo(fcdata_t *fd,tscdata_t *td,unsigned char tcline,bsapinfo_t *pinfo)
{
	...
	memset(pinfo,0,sizeof(bsapinfo_t));
	bsa_get_phase_info(fd,td,tcline,*(fd->slnum),pinfo);
	return SUCCESS;
}
